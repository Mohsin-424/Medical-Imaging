...
class CNNBaseModel(nn.Module):
    ...
    def save_checkpoint(self, path="", filename=None, epoch=0, metric_values=None):
        """
    ...
    def load_checkpoint(self, file_path):
        """
        Load the model's weights saved into the checkpoint filename 
        :arg
            file_path: path file where model's weights are saved
        """
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        return checkpoint['epoch'], checkpoint['metric_values']